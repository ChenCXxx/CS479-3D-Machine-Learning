...
class PointNetFeat(nn.Module):
    ...
    def forward(self, pointcloud):
        ...
        # [B,N,3] -> [B,3,N] -> [B,64,N] -> ... -> [B,1024,N]
        x = pointcloud.transpose(2, 1)  # [B,3,N]

        # Input Transform
        if self.input_transform:
            trans3 = self.stn3(x)  # [B,3,3]
            x = torch.bmm(trans3, x)  # [B,3,N] (3x3) x (3xN) = (3xN)
        else:
            trans3 = None
        ...
        # Feature Transform
        if self.feature_transform:
            trans64 = self.stn64(x)  # [B,64,64] 
            x = torch.bmm(trans64, x)  # [B,64,N] (64x64) x (64xN) = (64xN)
        else:
            trans64 = None
        ...
        return global_feature, trans3, trans64

class PointNetCls(nn.Module):
    # Classification model based on PointNet Architecture.
    ...
    def forward(self, pointcloud):
        ...
        """
        # TODO : Implement forward function.
        # [B,N,3] -> [B,1024] -> [B,num_classes]
        global_feature, trans3, trans64 = self.pointnet_feat(pointcloud)
        x = self.fc1(global_feature)
        x = self.fc2(x)
        logits = self.fc3(x)
        return logits, trans3, trans64
